...
export async function getAllChirps(sortDesc: boolean) {
  let dir = asc;
  if (sortDesc) {
    dir = desc;
  }
  const result = await db.select().from(chirps).orderBy(dir(chirps.createdAt));
  return result;
}

export async function getAllChirpsByAuthorID(
  authorID: string,
  sortDesc: boolean,
) {
  let dir = asc;
